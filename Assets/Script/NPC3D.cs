...
public class NPC3D : MonoBehaviour
{
    ...
    // New addition: Flag to ensure dialogue triggers only once
    ...
    private void OnTriggerEnter(Collider other)
    {
        // If the other object is the player and the dialogue has not been triggered
        if (other.gameObject.CompareTag("Player") && !hasTriggered)
        {
            ...
            if (!string.IsNullOrEmpty(talkToNode))
            {
                ...
                Debug.Log("Start dialogue");
                dialogueRunner.StartDialogue(talkToNode); // Start dialogue at the specified node
            }
        }
    }
    ...
    private void OnTriggerExit(Collider other)
    {
        // If the player leaves the trigger zone, stop tracking the canvas
        if (other.gameObject.CompareTag("Player"))
        {
            canvasActive = false;

            // Do not stop the dialogue, allowing it to continue playing
            // dialogueRunner.Stop(); <- Removed to keep dialogue running
        }
    }
}
